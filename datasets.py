...
def load_sentiment_140(num_words=None, num_rows=None, test_split=0.2, seed=100):
    ...
    sentiment_data = pd.read_csv("data/training.1600000.processed.noemoticon.csv",
                                    encoding='ISO-8859-1',
    ...
    # Only grab num_rows rows
    sentiment_data = sentiment_data.iloc[:num_rows]

    # Split training data
    x_train, x_test, y_train, y_test = train_test_split(sentiment_data["Text"].to_numpy(), sentiment_data["Sentiment"].to_numpy(), test_size=test_split, random_state=seed)
